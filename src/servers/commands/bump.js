...
exports.run = async (client, message, args) => {
  ...
  if (cooldown - (Date.now() - lastDaily) > 0) {
    return await msgError(`This Server can only be bumped once every 2 hours.\n\`${timeObj}\` Time Left...`, {
      channel: message.channel
    });
  } else {
    ...
    ctx.fillText("BUMPED", 600, 325);
    
    ctx.font = `bold 50px ${Fonts}`;
    ctx.fillStyle = "#888888"
    ctx.fillText(`You can bump again in: ${ms(cooldown, {verbose: true})}`, 482, 400);
    const attachment = new Discord.MessageAttachment(canvas.toBuffer(), `bumped.png`);
    const bumbedEmbed = new Discord.MessageEmbed()
      .setTitle(`<a:yes:833101995723194437> Bumped **${message.guild.name}**`)
      .setColor(`GREEN`)
      .setAuthor(message.author.username, message.author.avatarURL({
        dynamic: true
      }))
      .setDescription(`**You can bump again in: \`${ms(cooldown, {verbose: true})}\`**`)
      .setImage("attachment://bumped.png")

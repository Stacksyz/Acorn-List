...
exports.run = async (client, message, args) => {
  ...
  let lastDaily = findServer.boost;
  let timeObj = ms(cooldown - (Date.now() - lastDaily), {
    verbose: true
  });
  if (cooldown - (Date.now() - lastDaily) > 0) {
    return await msgError(`This Server can only be boosted once every 2 hours.\n\`${timeObj}\` Time Left...`, {
      channel: message.channel
    });
  } else {
    ...
    ctx.fillText("BOOSTED", 600, 325);
    
    ctx.font = `bold 50px ${Fonts}`;
    ctx.fillStyle = "#888888"
    ctx.fillText(`You can boost again in: ${ms(cooldown, {verbose: true})}`, 482, 400);
    const attachment = new Discord.MessageAttachment(canvas.toBuffer(), `boosted.png`);
    const boostedEmbed = new Discord.MessageEmbed()
      .setTitle(`<a:yes:833101995723194437> Boosted **${message.guild.name}**`)
      .setColor(`GREEN`)
      .setAuthor(message.author.username, message.author.avatarURL({
        dynamic: true
      }))
      .setDescription(`**You can boost again in: \`${ms(cooldown, {verbose: true})}\`**`)
      .setImage("attachment://boosted.png")
      .attachFiles(attachment);
    message.channel.send({embed: boostedEmbed});
    await serverData.updateOne({
      id: message.guild.id
    }, {
      $set: {
        boost: new Date().getTime()
      }
    })
    await serverData.updateOne({
      id: message.guild.id
